...
NAV2D.Navigator = function(options) {
  ...
  if (withOrientation === false){
    ...
    var addchar = document.getElementById("add_char");
    ...
    var delstat = document.getElementById("delstat");
    ...
    var mouseEventHandler = function(event, mouseState) {
      ...
      if (mouseState === 'down'){
        ...
        // if charger button is pressed
          if(flag == 1){
  	        chargerMarker = new ROS2D.NavigationImage({
    	        size : 2.5,
    	        image: '1.jpg',
    	        pulse : false,
            });

            chargerMarker.x =  positionVec3.x;
            chargerMarker.y = -positionVec3.y;
            chargerMarker.rotation = 270;
           	chargerMarker.scaleX = 1.0 / stage.scaleX;
            chargerMarker.scaleY = 1.0 / stage.scaleY;
            
            that.rootObject.addChild(chargerMarker);
            gotostat.disabled = false;
            delstat.disabled = false;
            flag = 0;
            addchar.disabled = true;

            logmsg.innerHTML += "<br /> Charging Station Added.";
          }
        ...
        if ((mouseDown === true)) {
          ...
          // if goal button is pressed
          if((goal.disabled == true)){
    ...
    gotostat.addEventListener("click", function() {
        ...
        positionVec3.x = chargerMarker.x;
        positionVec3.y = -chargerMarker.y;
        positionVec3.z = 0;
        
        var orientation = new ROSLIB.Quaternion({x:0, y:0, z:-0.7071067811865476, w:-0.7071067811865475});
        ...
        // send the goal
        sendGoal(pose);
        logmsg.innerHTML += "<br /> Going to Charging Station.";

        gotostat.disabled = true;
        
    });

    delstat.addEventListener("click", function() {
        that.rootObject.removeChild(chargerMarker);
        gotostat.disabled = true;
        delstat.disabled = true;
        addchar.disabled = false;
        logmsg.innerHTML += "<br /> Charging Station Deleted.";
    });
